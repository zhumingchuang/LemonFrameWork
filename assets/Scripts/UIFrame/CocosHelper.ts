import {
    Canvas, tween, Tween, Node, Animation, AnimationClip, Asset, resources, error, AssetManager, Rect, Camera, RenderTexture,
...
export default class CocosHelper {
    ...
    public static sleepSync = function (dur: number): Promise<boolean> {
        return new Promise((resolve, reject) => {
            // Canvas.instance.scheduleOnce(() => {    TODO: 暂未找到替代方法
            //     resolve(true);
            // }, dur);
        });
    }
    ...
    // public static async runActionSync(node: Node, ...actions: FiniteTimeAction[]) {   TODO: 暂未找到替代
    ...
    //     let rect: Rect = new Rect(0, 0, visibleRect.width, visibleRect.height);
    //     if (prop) {
